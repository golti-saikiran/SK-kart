...
const ResetPasswordStep: React.FC<ResetPasswordStepProps> = ({ onNext, email }) => {
    ...
    const validateAndSubmit = async () => {
        try {
            ...
            if (error instanceof yup.ValidationError) {
                ...
                setErrors({ ...errors, apiError: "Failed to reset password. Please try again." });
            }
        } finally {
            setLoading(false);
        }
    ...
    const errorMessage =
        errors.apiError || errors.newPassword || errors.confirmNewPassword;

    return (
        <>
            {isSuccess ? (
                <p className="success message">
                    Password reset successful! Now you can log in with the new password.
                </p>
            ) : (
